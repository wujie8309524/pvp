    ...
    public function __construct() {
        ...
        if($_GET['token'] /*&& !$_SESSION['uid']*/){
                ...
                if($user){
                    ...
                    session('user', $user);
                    cookie("user_login", $user['nickname'], 3600 * 24 * 30);
                }
        }

        /*if(!$_SESSION['uid'] && !$_GET['code']){
           $baseurl=urlencode('http://'.$_SERVER['HTTP_HOST'].$_SERVER['PHP_SELF'].'?'.$_SERVER['QUERY_STRING']);
           $url='http://aa.ri67g.cn/get-weixin-code.html?appid='.$this->extract['weixin_appid'].'&redirect_uri='.$baseurl.'&response_type=code&scope=snsapi_userinfo&state=123#wechat_redirect';
           header("Location: ".$url);
           exit;
        }
...
*/
    ...
    public function display($templateFile = '', $charset = '', $contentType = '', $content = '', $prefix = '') {
        ...
        var_dump($this->parseTemplate($templateFile));
