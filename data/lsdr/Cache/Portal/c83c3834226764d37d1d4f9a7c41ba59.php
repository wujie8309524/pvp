...
<link rel="stylesheet" type="text/css" href="http://33ss.topwk.cn/app/css/bull_vue-2.0.3.css">
...
function randomString(len) {
  ...
  var title  = '<?php echo ($room["roomid"]); ?>八倍九人房间';//分享的标题
  ...
  //var desc = '模式<?php echo ($room['wfname']); if($room['df']): ?>底分：<?php echo ($room['df']); endif; if($room['gz']): ?>规则： <?php echo ($room['gz']); endif; ?> <?php if($room['px']): ?>牌型： <?php if(is_array($room['px'])): foreach($room['px'] as $key=>$one): echo ($one); endforeach; endif; endif; ?> <?php if($room['sz']): ?>上庄<?php echo ($room['sz']); endif; ?> ';//分享的描述信息
   var desc = 'http://'+window.location.hostname+'/jf/bne/'+randomString(12);
...
"></div> 
   ...
   <div id="table" class="table">
    <img src="http://33ss.topwk.cn/app/img/bull9/table-8.jpg" class="tableBack" />
...
setTimeout("jzy.style.display='none'",100);
   	...
   	<div style="position: fixed;width:100%;height:100%;top:0;left:0;background: #000;z-index:99999999;" id="jzy">
		<div class="load4">
